...
test('sql queries optimization test', function () {
    ...
    expect(formatQueries(DB::getQueryLog()))
        ...
        ->sequence(
            ...
            fn ($query) => $query->toBe('select `id`, `log_name`, `description`, `subject_type`, `subject_id`, `subject`, `causer_type`, `causer_id`, `causer`, `event`, `created_at`, `updated_at` from `activity_log` where `id` = ? limit 1'),
        );
