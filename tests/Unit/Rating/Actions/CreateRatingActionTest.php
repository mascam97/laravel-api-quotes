...
it('can create a rating for a quote', function () {
    $rateData = new StoreRatingData(
        score: 5,
        rateableId: $this->quote->getKey(),
        rateableType: $this->quote->getMorphClass()
    );

    $rating = (new StoreRatingAction())->__invoke($this->user, $rateData);

    assertTrue($rating->qualifier()->is($this->user));
    assertTrue($rating->rateable()->is($this->quote));
...
test('sql queries optimization test', function () {
    ...
    (new StoreRatingAction())->__invoke($this->user, $rateData);
